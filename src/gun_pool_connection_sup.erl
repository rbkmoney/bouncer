-module(gun_pool_connection_sup).
...
-spec start_connection(inet:hostname() | inet:ip_address(), inet:port_number(), gun_pool_connection:client_opts()) ->
    {ok, pid()} | {error, _}.
start_connection(Host, Port, Opts) ->
    supervisor:start_child(?SERVER, [Host, Port, Opts]).

-spec stop_connection(pid()) ->
    ok | {error, term()}.
stop_connection(Pid) ->
...
init([]) ->
    ...
    Children = [#{
        id => connectionN,
        start => {gun_pool_connection, start_link, []},
        restart => transient,
